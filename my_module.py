...
class ChoroplethMap:
    ...
    def __init__(self, data_path, shapefile_path):
        ...
        try:
            ...
            # Data Cleaning 
            self.df.iloc[:, 2:] = self.df.iloc[:, 2:].apply(pd.to_numeric, errors='coerce')
            self.df = self.df.replace('*', pd.NA)  # Replacing '*' with NA
            self.df.iloc[:, 2:] = self.df.iloc[:, 2:].abs()  #
            self.df.fillna(self.df.mean(), inplace=True)
            ...
            print(f"Error initializing ChoroplethMap: {e}")
            return
    ...
    def replace_state_names(self, replace_dict):
        ...
        try:
            ...
            print(f"Error replacing state names: {e}")

    def randomly_select_columns(self, num_columns=15):
        """Randomly select columns from the DataFrame."""
        try:
            all_columns = self.df.columns[5:]  # Exclude the first five coloums as they are not survey data
            self.short_column_names = random.sample(all_columns.tolist(), num_columns)
        except Exception as e:
            print(f"Error randomly selecting columns: {e}")
    ...
    def group_and_merge_data(self):
        ...
        try:
            ...
            print(f"Error grouping and merging data: {e}")
    ...
    def plot_choropleth_map(self):
        ...
        try:
            ...
            print(f"Error plotting choropleth map: {e}")
    ...
    def execute_choropleth(self):
        ...
        while True:
            try:
                ...
                print(f"Error executing choropleth visualization: {e}")
